...
const Sortable = ({ img, id, uid, index, handleSelectItem, isSelected }) => {
   const { setNodeRef, attributes, listeners, transform, transition } = useSortable({ id: uid });
   ...
   return (
      ...
      >
         <div
            className={`absolute top-0 left-0 w-full h-full ${isSelected
               ? 'bg-white/50'
         ...
         >
            <input
               ...
               checked={isSelected}
               type="checkbox"
               className={`w-5 h-5 ${isSelected ? 'm-4' : 'm-2 sm:m-4 group-hover:block hidden'}`}
            />
         </div>
